 ...
 *
 * <p>Todos los endpoints de este controlador están protegidos con el rol
 * {@code ADMIN} a través de {@code SecurityConfig}. Proporciona las
 * operaciones CRUD completas sobre los usuarios de la plataforma.</p>
 *
 * <ul>
 *   <li>{@code GET    /api/admin/users}               → Lista todos los usuarios</li>
 *   <li>{@code POST   /api/admin/users}               → Crea un usuario manualmente</li>
 *   <li>{@code PUT    /api/admin/users/{id}}           → Actualiza datos de un usuario</li>
 *   <li>{@code PUT    /api/admin/users/{id}/toggle-block} → Bloquea/desbloquea un usuario</li>
 *   <li>{@code DELETE /api/admin/users/{id}}          → Elimina permanentemente un usuario</li>
 * </ul>
 */
@RestController
